...
//check total expired interest prefix
void Forwarder::check_counter()
{
  //caculate total counter
  for(auto it1=m_fib.begin();it1!=m_fib.end();it1++)
  {
    total_counter = total_counter + it1->getCounter();
	//it1->setCounter(0);
  }
  //check total counter bigger than threthold or not
  if(total_counter>=76)
  {
	for(auto it2=m_fib.begin();it2!=m_fib.end();it2++)  //add malicious interest prefix into MIL
    {
      int8_t random = rand()%100;
	  int8_t p = 100*(it2->getCounter()/(double)total_counter);
	  //first write
	  if(MIL.size()==0&&(random<p))MIL.push_back(it2->getPrefix());
	  else
	  {
	    //check if the MIL existed.
	    for(uint8_t it3=0;it3<MIL.size();it3++)
	    {
	      if(MIL[it3]==it2->getPrefix())break;
		  else if((it3==MIL.size()-1)&&(random<p))MIL.push_back(it2->getPrefix());
	    }	
	  }
	  it2->set_isattacker(false);   //update fib::entry:isattacker per second to avoid legitimate interest discarding 
    }
	/*NFD_LOG_DEBUG("AFTER MIL SIZE = " << MIL.size() <<" !!!");
	for(uint8_t it4=0;it4<MIL.size();it4++)  //compare MIL with LIL
	{
	  for(uint8_t it5=0;it5<LIL.size();it5++)
	  {
        if(MIL[it4]==LIL[it5])
		{
		  auto it6=MIL.erase(MIL.begin()+it4);
	      it4--;
		}
	  }
	}*/
  }
  //clear per prefix counter
  for(auto it4=m_fib.begin();it4!=m_fib.end();it4++)
  {
	it4->setCounter(0);
  }
  NFD_LOG_DEBUG("TOTAL COUNTER = " << total_counter <<" !!!");
  //clear total_counter
  total_counter = 0;
  NFD_LOG_DEBUG("MIL LIST =   !!!");
  NFD_LOG_DEBUG("MIL SIZE =   !!! "<<MIL.size()<<" !!!");
  for(uint8_t it6=0;it6<MIL.size();it6++)
  {
    NFD_LOG_DEBUG("MIL   = " << MIL[it6] <<" !!!");
  }   
  NFD_LOG_DEBUG("END MIL LIST =   !!!");
  /*NFD_LOG_DEBUG("LIL LIST =   !!!");
  NFD_LOG_DEBUG("LIL SIZE =   !!! "<<LIL.size()<<" !!!");
  for(uint8_t it7=0;it7<LIL.size();it7++)
  {
    NFD_LOG_DEBUG("LIL   = " << LIL[it7] <<" !!!");
  }   
  NFD_LOG_DEBUG("END LIL LIST =   !!!");*/
  ns3::Simulator::Schedule(Seconds(1.0), &Forwarder::check_counter, this);  //check total counter per second.
}
...
Forwarder::onIncomingInterest(Face& inFace,  const Interest& interest)
{
  ...
  //const std::vector<ndn::Name>& mlist = m_fib.findLongestPrefixMatch(interest.getName())->getm_list();
  //NFD_LOG_DEBUG("!!!!!!!!   @@@@@@@@@@ prefix is attack?" << m_fib.findLongestPrefixMatch(interest.getName())->get_isattacker());
  //NFD_LOG_DEBUG("!!!!!!!!	 @@@@@@@@@@ m-list = " << mlist.size() << "	@@@@@@ ");       //m-forward-interest
  //if(MIL.size()!=0)
  //{
     for(uint8_t it=0;it<MIL.size();it++)
	 {
	   if(MIL[it]==interest.getName().getPrefix(2))
	   {
	     NFD_LOG_DEBUG("!!!!!!!!	 @@@@@@@@@@ MIL = " << MIL[it] << "	@@@@@@ ");       //m-forward-interest
	     Name m_name = interest.getName();	 
         const Name after_append_name = m_name.appendNumber(inFace.getId()); 
		 Interest n_interest = interest;
		 shared_ptr<const Interest> m_interest = make_shared<Interest>(n_interest.setName(after_append_name)); //modiflied interest to m-interest
		 NFD_LOG_DEBUG("!!!!!!!!	 @@@@@@@@@@ after append name is = " << m_interest->getName() << "	!!!!!!!!     @@@@@@ ");
		 // PIT insert indirectly 
         shared_ptr<pit::Entry> pitEntry_indirect = m_pit.insert_indirect(interest).first;
		 this -> onM_listHit(inFace, pitEntry_indirect, interest);
		 return;
	   }
	 }
  //}
  //else this->onM_listMiss(inFace,interest);
  //{
      // PIT insert
...
void
Forwarder::onM_listHit(const Face& inFace, shared_ptr<pit::Entry> pitEntry, const Interest& interest)
{
	NFD_LOG_DEBUG("onM_listHit interest=" << interest.getName());

	NFD_LOG_DEBUG("onM_listHit shared_ptr<pit::Entry> pitEntry=  " << pitEntry->getName());
	...
	if (!interest.hasLink()) {
	  ...
	  NFD_LOG_TRACE("onM_listHit noLinkObject");
	}
	else {
	  ...
	  if (m_networkRegionTable.isInProducerRegion(link)) {
		// FIB lookup with Interest name
		fibEntry = m_fib.findLongestPrefixMatch(interest.getName());
		NFD_LOG_TRACE("onM_listHit inProducerRegion");
	  }
	  // has SelectedDelegation?
		...
		NFD_LOG_TRACE("onM_listHit hasSelectedDelegation=" << interest.getSelectedDelegation());
	  }
	  else {
		...
		if (isDefaultFreeZone) {
		  ...
		  for (const std::pair<uint32_t, Name>& delegation : link.getDelegations()) {
			...
			if (fibEntry->hasNextHops()) {
			  ...
			  NFD_LOG_TRACE("onM_listHit enterDefaultFreeZone"
		  ...
		  NFD_LOG_TRACE("onM_listHit inConsumerRegion");
		}
	  }
	}
                              ...
                              const time::milliseconds& dataFreshnessPeriod)
{
  ...
  if(!isSatisfied)
  {
//	shared_ptr<fib::Entry> m_entry = m_fib.findExactMatch(pitEntry->getName().getPrefix(2));
	shared_ptr<fib::Entry> m_entry = m_fib.findLongestPrefixMatch(pitEntry->getName());
    m_entry->setPrefix(pitEntry->getName().getPrefix(2));
	m_entry->c_expiredPIT();
	m_entry->set_isattacker(true);   //do not trust this fib::entry 
/*	if(m_check_counter)
	{
	  m_entry->check_counter();
	  m_check_counter = false;
	};*/
  }
 /* else   //add legitimate interest prefix into LIL
  {
     //first write
	 if(LIL.size()==0)LIL.push_back(pitEntry->getName().getPrefix(2));
	 else
	 {
	   //check if the LIL existed.
	   for(uint8_t it7=0;it7<LIL.size();it7++)
	   {
	     NFD_LOG_DEBUG("onInterestFinalize interest prefix=" << pitEntry->getName().getPrefix(2) << "  ");
	     if((LIL[it7]!=pitEntry->getName().getPrefix(2))&&(it7==LIL.size()-1))LIL.push_back(pitEntry->getName().getPrefix(2));
	   }
	 }
  }*/
...
Forwarder::setUnsatisfyTimer(shared_ptr<pit::Entry> pitEntry)
{
  ...
  pitEntry->m_unsatisfyTimer = scheduler::schedule(time::nanoseconds(2000000000),
